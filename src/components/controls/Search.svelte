...
const syncNavbarSearchState = (): void => {
	document
		...
		?.classList.toggle("search-expanded", isDesktopExpanded);
};

const expandDesktopSearch = async (): Promise<void> => {
...
const collapseDesktopSearch = (): void => {
	if (keywordDesktop) return;
	isDesktopExpanded = false;
	syncNavbarSearchState();
	document.getElementById("search-panel")?.classList.add("float-panel-closed");
};

const toggleDesktopSearch = async (): Promise<void> => {
	if (!isDesktopExpanded) {
		await expandDesktopSearch();
		return;
	}

	if (!keywordDesktop) collapseDesktopSearch();
};
...
<div
	id="search-bar"
	class:search-expanded={isDesktopExpanded}
	class="hidden lg:flex transition-all duration-300 ease-out items-center h-11 mr-0 rounded-lg overflow-hidden
      bg-black/4 hover:bg-black/6 focus-within:bg-black/6
      dark:bg-white/5 dark:hover:bg-white/10 dark:focus-within:bg-white/10"
>
    <button
        type="button"
        on:click={toggleDesktopSearch}
        aria-label={i18n(I18nKey.search)}
        aria-expanded={isDesktopExpanded}
        class="search-icon-button btn-plain scale-animation rounded-lg h-11 w-11 active:scale-90 shrink-0"
    >
    ...
    <input
        ...
        class="search-desktop-input transition-all duration-300 ease-out text-sm bg-transparent outline-0
         h-full w-0 min-w-0 text-black/50 dark:text-white/50"
    >
</div>
...
<style>
    ...
    #search-bar.search-expanded .search-desktop-input {
        width: 12.25rem;
        padding-right: 0.9rem;
    }

    #search-bar:not(.search-expanded) .search-desktop-input {
        opacity: 0;
        pointer-events: none;
    }
